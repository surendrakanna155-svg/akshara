...
STATEMENT_LANE = "STATEMENT_BASED"
...
def build_variant(ra: ResolvedAR, key: str) -> Optional[dict]:
    ...
    depth = 3
    # CONCEPT COUNT MUST BE THE ACTUAL COUNT. This was hardcoded to 2, which is true only for key (b) —
    # the option whose Reason is drawn from a DIFFERENT certified concept. Keys (a), (c) and (d) all state
    # a Reason about the item's own concept and therefore use exactly one. The final audit caught it:
    # 12 of 16 AR items carried concept_count=2 when they used 1, inflating the score from 0.28 to 0.38
    # and pushing them across the 0.30 easy/moderate threshold. A difficulty band that cannot be
    # recomputed from the item's own declared concepts is a stamped label, which is the thing this
    # architecture refuses everywhere else.
    n_concepts = 2 if key == "b" else 1
    diff = DIFF.predict(depth, n_concepts, misconception_pressure=0.0, calculation_load=0.2)
...
# JEE Main's current staple. Structurally it is the assertion-reason truth table with the explanation link
# removed: the candidate judges TWO independent statements and reports which are true. Because the
# explanation question is not asked, the option set is different and so is the key derivation — but both
# truths are settled by exactly the same computed probe, so nothing new has to be trusted.
...
def _statement_key(s1_true: bool, s2_true: bool) -> str:
    if s1_true and s2_true:
        return "a"
    if not s1_true and not s2_true:
        return "b"
    return "c" if s1_true else "d"


def build_statement_variant(ra: ResolvedAR, key: str) -> Optional[dict]:
    """One statement-based item whose key follows from two computed truths."""
    b = ra.binding
    # Statement I is always a claim about the concept's own relation; Statement II likewise. Their truth
    # values are chosen to realise `key`, then RE-DERIVED below rather than trusted.
    want1, want2 = {"a": (True, True), "b": (False, False),
                    "c": (True, False), "d": (False, True)}[key]
    s1 = b.true_claims[0] if want1 else b.false_claim
    s2 = b.true_claims[1 % len(b.true_claims)] if want2 else b.false_claim
    if s1.text == s2.text:
    ...
    if got1 is None or got2 is None:
        return None
    derived = _statement_key(bool(got1), bool(got2))
    if derived != key:
    ...
    steps = [
        ...
        f"{SOL.pretty_equation(b.relation)}: evaluating it before and after the stated change shows "
        f"Statement I is {'TRUE' if got1 else 'FALSE'}.",
        f"Step 2 — Test Statement II the same way: it is {'TRUE' if got2 else 'FALSE'}.",
        f"Step 3 — Two truths, judged independently, give option ({key}) {STATEMENT_OPTIONS[key]}.",
    ]
    rationale = {}
    facts = {"a": (True, True), "b": (False, False), "c": (True, False), "d": (False, True)}
    for lab, (w1, w2) in facts.items():
        if lab == key:
            continue
        why = []
        if w1 != bool(got1):
            why.append(f"it requires Statement I to be {'true' if w1 else 'false'}, but the relation "
                       f"makes it {'true' if got1 else 'false'}")
        if w2 != bool(got2):
            why.append(f"it requires Statement II to be {'true' if w2 else 'false'}, but the relation "
                       f"makes it {'true' if got2 else 'false'}")
    ...
    return {
        ...
        "archetype": "misconception_detection", "lane": STATEMENT_LANE,
        ...
        "reasoning": (
            ...
            "who is confident about one still has to evaluate the other. The format rewards knowing the "
            "relation well enough to predict how the quantity responds, and it gives no credit for "
            "recognising a familiar-looking sentence."),
...
def generate_statements(resolved: Sequence[ResolvedAR],
                        keys: Sequence[str] = ("a", "b", "c", "d")) -> List[dict]:
    out: List[dict] = []
    for ra in resolved:
        for k in keys:
            it = build_statement_variant(ra, k)
            if it is not None:
                out.append(it)
    return out


def verify_statement_options(item: dict) -> dict:
    """Every non-key option must be contradicted by a computed truth value."""
    tt = item["provenance"]["truth_table"]
    actual = (tt["statement_1_true"], tt["statement_2_true"])
    expected = {"a": (True, True), "b": (False, False), "c": (True, False), "d": (False, True)}
    key = item["answer_label"]
    unrefuted = [l for l, e in expected.items() if l != key and e == actual]
    ok = not unrefuted and expected[key] == actual
    return {"ok": ok, "unrefuted": unrefuted,
