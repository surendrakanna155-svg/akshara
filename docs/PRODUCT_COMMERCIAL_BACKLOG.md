...
| **Staff Face ID attendance** (O5) | 🔴 planned — **PARALLEL GA-BLOCKER** | No biometric capture wired | **NEW build:** staff check-in via device Face ID/biometrics. Student attendance stays teacher-entered (O4). New feature + RBAC + audit + cert. **Owner decision (2026-06-30, QW8): tracked as a SEPARATE parallel GA-blocker, NOT inside QW8's `QA-R` rows** — GA waits on BOTH QW8 Track B AND this cert. (`QA-R-012` is BLOCKED partly on this.) |
