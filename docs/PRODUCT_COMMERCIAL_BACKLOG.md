...
| O5 | **Staff attendance MAY use Face ID.** Staff Face ID is **Must Before GA**. ⚠ **CORRECTED 2026-07-01 (FINAL):** attendance auth = **GPS geofence → anti-mock location validation → live camera face verification → check-in/out**; **NEVER** device biometric / Touch ID / fingerprint / PIN / password (OS biometric = **app-login only**). As-built device-biometric O5 is **superseded** → re-implement (P1). See [`ATTENDANCE_AUTH_DESIGN_DECISION.md`](ATTENDANCE_AUTH_DESIGN_DECISION.md). | Owner (2026-06-30; corrected 2026-07-01) |
...
| **Staff Face ID attendance** (O5) | 🟢 **BUILT + locally certified** (`STAFF_FACE_ID_ATTENDANCE_CERTIFICATION.md`, 2026-06-30); live deploy pending | Was: no biometric wired | **BUILT** end-to-end: `local_auth` biometric **hard-block** (no PIN fallback) + both check-in/out + append-only `staff_check_ins` ledger (RLS self-insert) + `markStaffAttendance` RBAC (all staff personas) + audit + offline-queue + `StaffCheckInCard` on the HR Staff Attendance screen. flutter 3176/0 · deno 15/0 · analyze 0. **Owner decision (2026-06-30): separate parallel GA-blocker, NOT a `QA-R` row.** **Residual = Track B:** live migration apply + edge deploy + on-device biometric run. `QA-R-012` stays BLOCKED until that live leg is green. |
| **Backup & Disaster Recovery (backend)** | 🟢 local-certified (QW8 `QA-R-009`) | Backup/restore *health* logic certified 19/19; restore screen **relabeled operator-assisted** (no self-serve endpoint — owner decision); `BACKUP_RESTORE_RUNBOOK.md` restored | Live VPS `pg_dump→restore→integrity→drop` drill = **Track B** (pending SSH socket). Closes the QW5-deferred `QA-J-046`. |
...
| **QW8** Production Readiness & Market Certification (incl. backup/DR, security, perf) | 🟢 **LOCAL SCOPE COMPLETE** (`QW8_COMPLETION_CERTIFICATION.md`, 2026-06-30) — flutter 3164/0 · deno 47/0 · bash 21/0 · analyze 0; 126 new tests + 3 docs + restore relabel + 13 isolation probes + 2 live harnesses. 4 owner decisions (live-infra=run-for-real, pilot=representative-pass, restore=operator-assisted, Face ID=parallel-blocker). **GA gate `QA-R-012` BLOCKED** on: (1) Track B live run (SSH socket + tenant Postgres — harnesses staged); (2) staff Face ID cert; (3) live-regression cron 7-day green. |
