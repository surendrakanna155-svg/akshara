...
> **✅ Journey Wave 2 CLOSED (2026-06-26):** the 10 wire-gap-404 items — `MJ-C5` (teacher exam workflow), `MJ-H13` (teacher parent-comms/concerns), `MJ-C3` (parent messaging), `MJ-C4` (parent PTM), `MJ-H12` (parent leave read/write split), `MJ-M2` (student notifications), `MJ-H14` (admissions 5 tabs), `MJ-H15` (approval notes), `MJ-C6` (comms templates/history), `MJ-H16` (management settings PUT) — are **fixed, deployed, live-certified 28/28** (+35 deno tests; migrations `20260804000000`, `20260812000000`). See `docs/JOURNEY_WAVE_2_CERTIFICATION.md`. **RESOLVED**.

> **✅ Journey Wave 3 CLOSED (2026-06-26):** the 6 static-snapshot read items — Teacher `MJ-C7` (TEACH-1/5 roster/marks/leave/dashboard overlay), Principal `MJ-C8` (PRINC-1 exec dashboards), HR `MJ-H17` (HR-5 KPIs + real AI insight), Alumni `MJ-H18` (ALUMN-2 dashboard aggregates) + `MJ-M3` (ALUMN-1 profile real/honest), Library `MJ-M4` (LIBRA-6 dashboard/fines/reports + activeLoans) — now **compute from live tables inside tenant RLS, not seed; fixed, deployed, live-certified 34/34** (analyze 0 · flutter 2389 · deno 790, +48 new tests; no migrations). Live cert caught + fixed a live-only `GROUP BY` 500 in the teacher classes aggregate (mock tests can't reproduce Postgres grouping = TEACH-6). See `docs/JOURNEY_WAVE_3_CERTIFICATION.md`. **RESOLVED**.

> **✅ Journey Wave 4 CLOSED (2026-06-27):** the 12 write-surface/orphaned-feature items — `MJ-C9` (inventory vendor-create + real-vendor PO), `MJ-H23` (Employee Platform nav), `MJ-H24` (Promotion/Holiday/Meta nav), `MJ-M5` (onboarding wizard nav), `MJ-M8` (HR dialogs honor input), `MJ-H19` (transport attendance = TRANS-1), `MJ-M9` (transport delay-notify + SIS identity = TRANS-2/3), `MJ-H20` (hostel attendance+mess = HOSTE-1/3), `MJ-H21` (library member/issue-validation/fines/resource = LIBRA-1/2/4), `MJ-H22` (alumni donations = ALUMN-3), `MJ-M6` (Org Builder REAL provisioning = OB-1), `MJ-M7` (admissions real Storage = ADMIS-5) — are **fixed, deployed, live-certified 28/28** (2 migrations + 21 edge files). See `docs/JOURNEY_WAVE_4_CERTIFICATION.md`. **RESOLVED**.

> **✅ Journey Wave 5 CLOSED (2026-06-27):** the 10 RBAC-hardening/error-state/test-parity items — `MJ-M10` (teacher writes gate on granular perms = TEACH-4), `MJ-M11` (parent-scoped attendance correction + marking gate = ATTEN-2/3), `MJ-L2` (per-widget RBAC + drill-down + hidden-widget fetch = DW-1/2/3), `MJ-L4` (copilot error state + transport fallback = AF-2/3), `MJ-L5` (superAdmin unseeded-perm drift + honest mock-write state = SA-1/2), `MJ-L3` (Director entitlement gates + write error handling = DIREC-1/2/3), `MJ-L6` (refund self-approval block = FINAN-5), `MJ-L7` (inventory intelligence side-effect-free reads = INVEN-6), `MJ-L8` (global-search RBAC filter = ADMIN-6), `MJ-M12` (comms path-parity tests = COMMU-3) — are **fixed, deployed, live-certified 24/24** (migration `20260807000010`; analyze 0 · flutter 2439 · deno 848). Live cert caught + fixed 3 real defects (pre-existing `recorded_on` 500 in the operations-hub/widgets endpoint; parent-correction PK collision under RLS-filtered count; error-mapping masking). MJ-M12 also found + fixed a real missing `PUT /communications/templates/:id` handler. Class-assignment binding (TEACH-4 half) deferred pending ATTEN-7 identifier normalization. See `docs/JOURNEY_WAVE_5_CERTIFICATION.md`. **RESOLVED**.
