...
**Date:** 2026-07-07 · **Status:** 🟢 **APPROVED — 🔒 Program Baseline v1.0 (owner, 2026-07-07). D-1..D-6 resolved; implementation authorized starting at §8 row 1.**
...
3. **MCIP spec** (`spec/MASTER_CURRICULUM_INTELLIGENCE_PIPELINE.md`, Parts 01–16) + **engine addendum** (`spec/DOWNLOAD_VERIFICATION_AND_RECOVERY_ENGINE.md`).
4. This program's audits + planning suite (conflict resolutions in `audits/GAP_ANALYSIS.md` §2 are settled — do not relitigate).
...
- **Risk watch:** top items R1 legal/copyright (never store out-of-scope question content; license status mandatory), R3 solver regression, R7 dataset poisoning (adversarial verify before any code wave consumes a dataset), R10 binaries-in-git (guard exists — keep it). Full register: `planning/RISK_REGISTER.md`.
...
| **Highest-risk modules** | CI-C1 solver upgrade (touches the certified path — golden-first, template-absent-legacy) · CI-C6 ingestion (OCR quality + token discipline) · anything near D-3 legal scope |
...
| **Expected timeline** | Data lane ~39–61 dev-days elapsed (external-bounded; checkpointed) · P1-CI-0 ~2–4 days · v3.0 Phase-1 code waves ~30–40 days post-pilot · program G2 ≈ data-lane length since code fits inside it |
