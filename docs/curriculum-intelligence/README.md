...
- **🔒 Local-storage lock (owner 2026-07-08, [`planning/CONTENT_DEPENDENCY_MAP.md`](planning/CONTENT_DEPENDENCY_MAP.md) §6):** raw PDFs = **local only + gitignored**; derived/AI-generated knowledge (blueprint JSONs, catalogue, metadata, indexes) = **local only, NOT committed yet** (repository-scale commit decided later); **Git commits ONLY schemas · engine · manifests · tests · code**. **VPS = application server ONLY**; **no production DB import / VPS storage dependency for curriculum yet**. Engine waves build schema (dormant) + a generic representative **test fixture** + tests, never seed the production DB.
