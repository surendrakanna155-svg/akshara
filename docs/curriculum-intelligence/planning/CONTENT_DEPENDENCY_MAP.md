...
|---|---|---|
| 🟢 **RUNNABLE NOW** (content-independent, deterministic, additive) | **✅ CI-C1 DONE** (2026-07-08, `31d22f96` — `edu_blueprint_templates` dormant schema + template-aware solver, built on the real local CBSE blueprint) · **CI-C8** (exposure/rotation + selection-reason) · **CI-C3** (multi-set + PDF v2 + JSON export) · **CI-C7** (Exam Profile Engine + tier gating) · **CI-C4-schema** (outcome/competency nullable columns + rule-first classifier) · **B12 family schema** (`edu_question_templates`/`edu_question_families`/`edu_distractors`, dormant) · **CI-E1b dormant seed** · **all APIs / DB models / tests / docs**. Option A **superseded** (owner 2026-07-08). **Next = CI-C3 ∥ CI-C7** (dep CI-C1, file-disjoint). |
