...
|---|---|---|
| 🟢 **RUNNABLE NOW** (content-independent, deterministic, additive) | **CI-C8** (exposure/rotation + selection-reason) · **CI-C1** (blueprint-template schema + template-aware solver upgrade, on a representative/synthetic template) · **CI-C3** (multi-set + PDF v2 + JSON export) · **CI-C7** (Exam Profile Engine + tier gating) · **CI-C4-schema** (outcome/competency nullable columns + rule-first classifier) · **B12 family schema** (`edu_question_templates`/`edu_question_families`/`edu_distractors`, dormant) · **CI-E1b dormant seed** (canonical-concept tables, migrations only) · **all APIs / DB models / tests / docs** for the above | none — runs on fixtures. ⚠ pulling these pre-pilot **reverses Option A** (owner confirm) |
...
is an owner decision surfaced in §4.*
