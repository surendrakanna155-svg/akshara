...
| **Current milestone** | **M0 ✅ · CI-A0 ✅ · P1-CI-0 ✅ · CBSE Class-X SQP acquisition ✅ · CI-C1 ✅ · CI-C3 ✅ · CI-C7 ✅ (2026-07-08, `2379a529`)**. Option A **superseded** (owner) → engine layer runs pre-pilot. **Next CI code = CI-C8** (paper↔exam exposure/rotation + selection-reason, zero-dep) **then CI-C4-schema** (single agent in `education/**`). Full-board acquisition (AP/TS/CISCE + broader CBSE) ⏳ owner/network-gated. AI waves (C5/C6/C10-engine/C11) gated on P3-AI-1 + concepts. **🔒 curriculum data local-only; VPS = app-server only.** |
