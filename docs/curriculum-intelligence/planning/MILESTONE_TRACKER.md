...
| **CI-C8** | **EOS FEATURE PASS** — additive/dormant, byte-identical when absent, certified goldens byte-stable, no E1a duplication, I1–I8, no prod seed/VPS | commit `e62c367c`; `edu_item_rotation_policies` dormant migration + `education_item_rotation.ts` (rotation-cooldown, collapses to canonical order when absent) + `education_selection_reason.ts` (read-only explainability); education deno **-A 113/0** (94+19) | 2026-07-08 | **First WORKTREE-ISOLATED agent** — integrated via clean cherry-pick, re-verified in-tree. **Next CI code = CI-C4-schema** (outcome/competency nullable columns + rule-first classifier; single agent in `education/**`). |
