...
- **P2-4 — Persona-cohesive premium surfaces ✅** — the greeting hero / AI bar / premium canvas came from the **fixed** indigo→violet `AksharaPremiumTokens`, so a blue Parent got a *violet* hero — incohesive with the branded nav. New `AksharaPremiumTokens.forAccent(accent)` re-tones every accent-dependent surface (hero + canvas gradients, AI-bar gradient, brand, line-art stroke, and a soft **accent-tinted** hero shadow in light) to the persona hue while keeping the neutral premium surface/border/on-hero (so light heroes stay pale → dark on-hero text still clears contrast). Wired via `_build(accent:)`: `persona()` passes its accent; `light()/dark()` pass `whiteLabel?.primary`. Parent=blue, Student=emerald, Teacher/Admin=indigo heroes now match their chrome. New `ds_v2_persona_hero_golden_test.dart` proves it. **Note:** existing dashboard goldens render via `AksharaAppTheme.light()/.dark()` (no persona), so they still show the base brand and did **not** change — a pre-existing test-fidelity gap (goldens don't exercise persona theming); candidate for a later persona-themed dashboard golden pass. analyze clean; full golden 73/73.
