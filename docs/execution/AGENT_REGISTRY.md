...
| `buewnbpvb` (bg) | bash | CONTINUOUS acquisition service (clean re-crawl) | ✅ ENGINE clean / ⚠ REPO INCOMPLETE | ENGINE proven (3 no-new + frontier drained; 1728 verified; integrity 2 not-on-disk was 1172; 1710 PDFs/6.8G; manifest `c8f6b6bf`) — but the REPO does NOT satisfy the canonical `Board→Class→Subject→DocType` matrix (CBSE-heavy, AP/TS thin, **CISCE 0**). Owner phrasing: "Acquisition engine complete. Curriculum repository still incomplete." Acquisition ≠ done until the matrix is filled |
