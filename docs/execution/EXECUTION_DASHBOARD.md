...
| **Current Wave** | **TWO ∥ lanes** (owner integrated Curriculum Intelligence, Baseline v1.0). **App lane: P1-PROD-17 — C19 · Parent self-service (PAR-1..5)** — 🔵 next up. **CI-DATA lane: CI-A0 remainder (finishing) → CI-A1 (CBSE)** in `curriculum/` (∥, disjoint tree, data-quality gate). **C6 + C13-Homework-half defer** (HWK-1 schema). **C3 defers** (GA-1 live); **C4/C5/C7/C8/C9/C10/C11/C12/C13-Exams/C14/C15/C16/C17/C18 ✅**. **P2-UX-1 ∥-eligible**. P1-CODE-4 stays 👤-gated. |
...
| **Last commit-gated wave** | **P1-PROD-16** (`c2306daa`) — C18 Leadership productivity: PRI-2 (unsubmitted-marks card), PRI-3 (daily report → PDF on XCT-1), DIR-1 (league table `rankDirectorSchools`), DIR-2 (collection report + XCT-1 exporters) all verified built (pre-wave `b1dd65c4`/`1e7e7186`); DIR-1/2 already tested. Closed the two verified test-coverage gaps: PRI-2 card (×3) + PRI-3 export (×2). All read-only (approval maker-checker untouched); no migration; test-only (0 `lib/`). analyze 0 · 2 files 7/7 · +5 tests · suite otherwise unchanged (3643 / 2 known UX-7). |
...
> **P1-PROD-16 (C18 Leadership productivity) ✅ 2026-07-07** (`c2306daa`) — verify-mostly; PRI-2/3 + DIR-1/2 all built (pre-wave), closed the two verified test-coverage gaps (PRI-2 card ×3, PRI-3 export ×2). **Sixteen C-waves now landed** (C1/C2/C4/C5/C7/C8/C9/C10/C11/C12/C13-Exams/C14/C15/C16/C17/C18).
>
> **▶ NEXT — TWO ∥ lanes** (owner integrated the Curriculum Intelligence program, Baseline v1.0):
> **(1) App lane: P1-PROD-17 (C19 — Parent self-service: PAR-1 PTM RSVP · PAR-2 Apply-Leave quick action · PAR-3 medical-cert upload · PAR-4 payment-history export · PAR-5 in-app reminder banners [XCT-2])** — discovery-first (parent-portal surfaces likely exist; verify/close only gaps). Tripwire: parent RLS = own child(ren) only; money read-only.
> **(2) CI-DATA lane: CI-A0 remainder (finishing) → CI-A1 (CBSE discovery)** in `curriculum/` (∥, disjoint tree, owner rule D-2; data-quality gate not EOS; no bulk copyrighted download). Spec-check done — AIMS update introduces NO mechanical change to CI-A0/A1/P1-CI-0 (deltas land in CI-B4 + new CI-C10/C11). **P1-CI-0** (code) lands before P4-RT-0.
