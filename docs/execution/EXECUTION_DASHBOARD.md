...
| **Current Wave** | **P1-PROD-15 — C17 · Library & Communication productivity (LIB-3/4/5, COM-3/4/5)** — 🔵 next up. **C6 + C13-Homework-half defer** (HWK-1 schema). **C3 defers** (GA-1 live); **C4/C5/C7/C8/C9/C10/C11/C12/C13-Exams/C14/C15/C16 ✅**. **P2-UX-1 ∥-eligible**. P1-CODE-4 stays 👤-gated. |
...
| **Last commit-gated wave** | **P1-PROD-14** (`5eba37d`) — C16 Transport & Inventory: TRN-5/6/7 + INV-3/4/6 verified built; closed TRN-8 (XCT-2 rail + latent `"staff"` audience CHECK defect fixed, regression-guarded), INV-7 (`storekeepers` audience via migration `20260851` — also restores `all_staff` dropped by `20260838` — storekeeper role seeded, all-staff fallback, SAVEPOINT-isolated dedupe), INV-5 GRN export. Parallel Transport∥Inventory build under disjoint ownership. analyze 0 · deno transport 40/0 · inv_finance 29/0 · comm 104/0 · exams 124/0 · +17 tests · suite 3643 pass, no-new-fail (2 known UX-7) |
...
>
> **▶ NEXT: P1-PROD-15 (C17 — Library & Communication productivity: LIB-3 barcode quick issue/return [plain barcode/ISBN, NOT biometric] · LIB-4 loan renewal/re-issue · LIB-5 overdue-book reminder [XCT-2] · COM-3 resend-to-unread · COM-4 schedule-send [activates dormant `scheduled_at` — XCT-2] · COM-5 save-broadcast-as-template)** — discovery-first (library issue/return + communication broadcast surfaces exist from C9; verify/expand, don't duplicate). LIB ∥ COM disjoint-ownership eligible, but LIB-5/COM-4 both touch the rail/communication module — split ownership at discovery. **P2-UX-1 ∥-eligible.** Still owner-gated: the identity batch (P1-CODE-4), module-scope 👤s (P1-CODE-6/7/8), and the live lane (`P0-INFRA-1/3`, `P0-TEST-1/2/3`).
