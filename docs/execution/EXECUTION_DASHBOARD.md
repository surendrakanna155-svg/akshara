...
| **Current Wave** | **TWO ∥ lanes** (owner integrated Curriculum Intelligence, Baseline v1.0). **App lane: P1-PROD-16 — C18 · Leadership productivity (PRI-2/3, DIR-1/2)** — 🔵 next up. **CI-DATA lane: CI-A0 remainder → CI-A1 (CBSE)** in `curriculum/` (∥, disjoint tree, data-quality gate). **C6 + C13-Homework-half defer** (HWK-1 schema). **C3 defers** (GA-1 live); **C4/C5/C7/C8/C9/C10/C11/C12/C13-Exams/C14/C15/C16/C17 ✅**. **P2-UX-1 ∥-eligible**. P1-CODE-4 stays 👤-gated. |
...
| **Last commit-gated wave** | **P1-PROD-15** (`d5edab9c`) — C17 Library & Communication: LIB-3/4 + COM-3/4/5 verified built (COM-4 `scheduled_at` already active — only the cron trigger remains = live-lane; COM-5 audience reuse = the separate `comm_audience_segments`, not a gap); closed **LIB-5** — re-pointed `handleSendOverdueReminders` off the IMMEDIATE blast-all-parents send onto the XCT-2 rail (`scheduleReminder` via testable core `runOverdueReminder`, TRN-8 pattern) — ONE `all_parents` digest, corrected copy, reminderId in audit. Backend-only. analyze 0 · deno library 59/0 (+2) · reminders 5/0 · comm 104/0 · flutter unchanged (0 Dart touched). |
...
>
> **P1-PROD-15 (C17 Library & Communication) ✅ 2026-07-07** — verify-mostly; the one genuine gap closed was **LIB-5** (re-pointed onto the ONE XCT-2 reminder rail via `runOverdueReminder`, TRN-8 pattern). Fifteen C-waves now landed (C1/C2/C4/C5/C7/C8/C9/C10/C11/C12/C13-Exams/C14/C15/C16/C17).
>
> **▶ NEXT — TWO ∥ lanes** (owner integrated the Curriculum Intelligence program, Baseline v1.0):
> **(1) App lane: P1-PROD-16 (C18 — Leadership productivity: PRI-2/3 · DIR-1/2)** — discovery-first (leadership surfaces exist from C10 Approval Center + intelligence dashboards; verify/close only gaps; pull exact scope from `FINAL_QA_ROADMAP.md` §Phase C).
> **(2) CI-DATA lane: CI-A0 remainder → CI-A1 (CBSE discovery)** in `curriculum/` (∥, disjoint tree, owner rule D-2; data-quality gate not EOS; no bulk copyrighted download). Read the updated canonical specs (`MASTER_CURRICULUM_INTELLIGENCE_PIPELINE.md` + `ASSESSMENT_INTELLIGENCE_MASTER_SPECIFICATION.md`) before CI implementation. **P1-CI-0** (code) lands before P4-RT-0.
> **P2-UX-1 ∥-eligible.** Still owner-gated: the identity batch (P1-CODE-4), module-scope 👤s (P1-CODE-6/7/8), and the live lane (`P0-INFRA-1/3`, `P0-TEST-1/2/3`).
