...
| **Current Wave** | **P1-PROD-14 — C16 · Transport & Inventory productivity (TRN-5/6/7/8, INV-3/4/5/6/7)** — 🔵 next up. **C6 + C13-Homework-half defer** (HWK-1 schema). **C3 defers** (GA-1 live); **C4/C5/C7/C8/C9/C10/C11/C12/C13-Exams/C14/C15 ✅**. **P2-UX-1 ∥-eligible**. P1-CODE-4 stays 👤-gated. |
| **Wave Status** | **P1-CODE-1/2/3/5 ✅ + P1-PROD-0 ✅ + C1/C2/C4/C5/C7/C8/C9/C10/C11/C12/C13-Exams/C14/C15 ✅** — through Finance, Exams, Registers/Certs, HR Payroll, Transport, Inventory/Library/Communication, Approval batch, Admissions, Finance receipting, Exams half, Teacher & Attendance, and HR & SIS productivity (HR-3 leave SoD + SIS-2/5 exports). P0: 14/19 ✅; 5 live-lane ⏳ owner-deferred. |
| **Planning** | 🔒 FROZEN 2026-07-04 → **AUTONOMOUS EXECUTION UNDER WAY** (P1 code + PROD waves; live lane deferred) |
| **Last commit-gated wave** | **P1-PROD-13** (`4b65ce1`) — C15 HR & SIS: HR-3/4/7 + SIS-2/5 verified built; closed HR-3 leave SoD (deny self-approval, approver ≠ filer, single+batch, 403), SIS-2 class-list + contact-sheet exports, SIS-5 exit-reason (read-only join to latest TC reason). Parallel HR∥SIS build under disjoint ownership. analyze 0 · HR deno 90/0 · SIS deno 139/0 (+1 pre-existing ISO-COUNT) · +~11 tests · suite no-new-fail (2 known UX-7) |
...
| P1 — Backend & Code Fixes | 35 | 17 (CODE-1/2/3/5, PROD-0, C1, C2, C4✓, C5, C7, C8✓, C9, C10, C11, C12, C13-Exams, C14) | 0 | 18 (next: C15 HR/SIS — C3+C6+C13-HW defer; CODE-4 👤) | EOS per wave |
...
> **P1 lane through P1-PROD-11 ✅ (2026-07-06).** Eleven C-waves landed (C1/C2/C4/C5/C7/C8/C9/C10/C11/C12/C13-Exams) atop P0·W1+legs, P1-CODE-1/2/3/5, and P1-PROD-0 (XCT). Latest: **P1-PROD-11 (C13 Exams half `0a8c2a3`)** — discovery-first verified EXM-4 (merit+topper), EXM-5 (pass-fail+grade-dist), EXM-7 (datesheet PDF) built; closed **EXM-6** — the **first caller of the XCT-2 reminder rail**: `handleRemindPendingMarks` finds exams past `marks_entry_deadline` with pending marks (checked at trigger → no false reminder) and schedules one in-app `all_teachers` reminder; deadline + Overdue UI on the progress board. Exam-result governance untouched. analyze 0 · deno exam 124/0 + reminders/audit 24/0 · +12 tests · suite no-new-fail (2 known UX-7). Homework half defers with C6.
>
> **▶ NEXT: P1-PROD-12 (C14 — Teacher & Attendance productivity: TCH-1 attendance-from-schedule-row · TCH-2 marks-pending on teacher home [XCT-2, reuse EXM-6 signal] · TCH-3 class-summary export · TCH-4 substitution alert + weekly timetable · ATT-3 absentees-only fast-mark · ATT-4 office not-yet-marked monitor)** — discovery-first (teacher home / timetable / attendance-marking surfaces exist; verify/expand, don't duplicate). Attendance integrity intact. **P2-UX-1 ∥-eligible.** Still owner-gated: the identity batch (P1-CODE-4), module-scope 👤s (P1-CODE-6/7/8), and the live lane (`P0-INFRA-1/3`, `P0-TEST-1/2/3`).
