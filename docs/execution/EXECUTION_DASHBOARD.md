...
| Field | Value |
|---|---|
| **Current Phase** | **🎉 P2 — UI/UX COMPLETE (2026-07-08)** — P2-UX-1..5 ✅ all done. **Two lanes now run in PARALLEL** (owner policy 2026-07-08): **(A) ERP** → next = **P3 Adaptive AI** (P3-AI-1) ⏸ owner-timing-gated; **(B) Curriculum (ACTIVE)** → Option A superseded, engine layer running pre-pilot: **CBSE Class-X SQP acquisition** (real, local-storage-locked) → CI-B3 → **CI-C1**. Remaining P1 = owner-gated P1-CODE-4/6/7/8; P0 code/security ✅ 14/19, 5 live-lane ⏳ owner-deferred. **🔒 Curriculum data stays local+Git; VPS=app-server only; no prod curriculum import yet.** 🟡 A2 pending ratification (D-7/I9 approved). |
| **Current Wave** | **P2-UX-4 ✅ COMPLETE (2026-07-08, `d412b78b`) — accessibility pass; closes UX-7.** Verification+lint layer over already-built a11y primitives (Polish §7): screen-reader semantics (unlabeled app-bar button fixed) · tap-target 48dp lint + guideline · dynamic-type 1.3× dense-grid clamp · rendered contrast audit beyond the token-pair gate (found+fixed a dark-mode badge ≈1.7:1 failure). analyze 0; suite 3756/0-fail (2 known UX-7 RESOLVED); goldens 70/70 unchanged; +21 a11y tests. **Next = P2-UX-5 (dark-theme user toggle, LAST P2 sub-wave) ⏸ owner-confirm.** Prior: P2-UX-1/2/3 ✅ COMPLETE (2026-07-08); P2-UX-3 = Tier-3 DS enforcement (`64613e07`). P2-UX-3 = Tier-3 DS enforcement (`64613e07`, pure-additive): DS lint guard (blocks new raw-color/TextStyle violations via ratchet baseline) · contrast checker gate (WCAG AA light+dark) · 4 persona-shell goldens. CI-wiring leg owner/CI-gated (deferred). Earlier this wave — P2-UX-2 "five daily tasks" done — quick-wins (`88d76df1` haptics · `723927b7` A5 nav + A8 keyboard · `fb781751` C2 density + SIS toggle · `fec526e5` fee ceremony) + 4 heavy builds: `ce45cf87` fee-counter §2.4 (real dues + PSID search + amber `AksharaQueuedView`) · `b34f2116` Approvals-Inbox §2.3 (**server-owned `sodBlocked`** flag + badge + swipe + grouping) · `87841013` attendance exception grid §2.1 · `43c8f8df` shared marks grid §2.2/2.5 (ONE kit, both chains). EOS UX PASS. **Remaining P1 owner-gated: P1-CODE-4/6/7/8.** **CI-DATA: CI-A0 ✅; CI-A1 ⏳ owner/network-gated**. |
...
| **Last commit-gated wave** | **P2-UX-5 → COMPLETE** (`dd471f2e`) — **Dark-theme user toggle · 🎉 Phase P2 EXIT.** Discovery-first: the toggle is already built + wired (theme_mode storage/provider · appearance-settings Light/Dark/System · `MaterialApp.themeMode` · routed · tested; default Light) → no rebuild. Closed the carried dark `tertiary` contrast gap (`tertiaryContainer` dark `#134E4A`→`#0F3D38`: **2.53→3.21:1**), raised the rendered-contrast-audit pin to the uniform 3.0 floor (all semantic tones clear WCAG AA in BOTH schemes), re-baselined 4 dark parent-dashboard goldens. analyze 0; theme 82/82; goldens 70/70; full suite **3756 pass / 1 skip / 0 fail**. Prior: **P2-UX-4 → COMPLETE** (`d412b78b`) — **Accessibility pass; closes UX-7.** Verification+lint layer over already-built a11y primitives (Polish §7 / C-ISS-13) + the genuine gaps it surfaced: **screen-reader semantics** (`AksharaAppBarIconButton` now announces `semanticLabel ?? tooltip` — was unlabeled) · **tap-target lint** (theme-48dp + `androidTapTargetGuideline`/`labeledTapTargetGuideline`) · **dynamic-type** shared `clampDenseGridTextScale` (cap 1.3× — the only permitted clamp) on the attendance exception grid + marks cell · **rendered contrast audit beyond the token-pair gate** (7 KpiAccent tones × badge/tile + alpha-blended freshness chip, light+dark) which **found + FIXED a dark-mode badge white-on-bright-tone ≈1.7:1 failure** via `onColorFor` · **UX-7** width-gated quick-action aspect (golden-safe, viewports ≥390px). analyze 0; full suite **3756 pass / 1 skip / 0 fail** (**2 known UX-7 RESOLVED**); goldens **70/70 unchanged**; +21 a11y tests. 1 tracked → P2-UX-5 (dark `tertiary` chip ≈2.5:1). Prior: **P2-UX-3 → COMPLETE** (`64613e07`) — Tier-3 design-system ENFORCEMENT, pure-additive (tests + goldens only, 0 `lib/`): **DS lint guard** (`ds_enforcement_test.dart` — scans `lib/features/**`, blocks new violations via a committed ratchet: hex `Color(0x` locked 0 · `Colors.<swatch>` ≤60 · `TextStyle(` ≤159) · **contrast checker gate** (`contrast_checker_test.dart` — `criticalSchemePairs` clear WCAG AA in light+dark + calibration) · **4 persona-shell goldens** (admin/teacher/parent/student chrome at 390×844; deterministic — QA-login off). analyze 0; theme+golden 142 pass (+22 gates). CI-wiring leg owner/CI-gated. Prior in P2 — Four heavy builds atop the quick-wins: **§2.4 fee counter** `ce45cf87` (real dues prefill kills the hardcoded `5000` · dues-breakdown line · PSID/name/class "Collect fee" origination · shared `AksharaQueuedView` amber offline receipt) · **§2.3 Approvals Inbox** `b34f2116` (**server-owned `sodBlocked`** maker-checker flag — no client SoD · badge + Approve-disable + swipe + type-grouping/critical-sort + on-card facts; deno approval 62/0; 3 goldens deliberate) · **§2.1 attendance exception grid** `87841013` (compact tap-cycle grid + single sticky present/absent/late bar; same marking workflow) · **§2.2/2.5 shared marks grid** `43c8f8df` (`lib/shared/marks_grid/` kit both chains render; teacher upgraded, certified admin untouched). analyze 0 · full suite **3711 pass / 2 known UX-7 (0 new)** · +~40 tests. EOS UX PASS (`EOS_P2-UX-2_FEATURE_REPORT.md`). |
...
| P2 — UI/UX | 5 | **5 ✅ (P2-UX-1..5) — 🎉 COMPLETE** | 0 | 0 | EOS UX per wave |
...
- **Implementation EOS runs so far:** **39** — P0·W1 DOCS · P0·W2 legs · P1-CODE-1/2/3/5 · P1-PROD-0 FOUNDATION · P1-PROD-1..19 (C1–C21) · **P1-CI-0** (curriculum-engine seam) · **P2-UX-1..4 → COMPLETE** · **P2-UX-5 → COMPLETE (dark-theme toggle; 🎉 Phase P2 EXIT) PASS (2026-07-08, latest)**. See `docs/engineering/eos/EOS_RUN_LEDGER.md`.
...
>
> **P2-UX-1 (feel & trust pack) ✅ 2026-07-08** — slice 1 (`aeb79938`) + slice 2 (5 sub-commits `e25d1282`→`b316e849`): `AksharaSkeleton` on 11 dashboards · A2 freshness chip (timer-safe connectivity) · A3 tabular figures on KPIs · A4 pull-to-refresh · draft chip + PopScope · copy/permission dictionary · A9. EOS UX PASS; analyze 0; suite 3686/2-known-UX-7; 4 goldens deliberate (A2 chip). Residual (tracked): raw `.when()` bulk migration → P2-UX-2 grid pass.
>
> **P2-UX-2 (the five daily tasks) ✅ 2026-07-08** — quick-wins (`88d76df1`·`723927b7`·`fb781751`·`fec526e5`) + 4 heavy builds: §2.4 fee counter `ce45cf87` (real dues prefill kills the hardcoded `5000` + PSID/name/class "Collect fee" origination + `AksharaQueuedView` amber offline receipt) · §2.3 Approvals Inbox `b34f2116` (**server-owned `sodBlocked`** flag — no client SoD · badge/Approve-disable/swipe/grouping; deno approval 62/0; 3 goldens deliberate) · §2.1 attendance exception grid `87841013` (compact tap-cycle + single sticky present/absent/late bar) · §2.2/2.5 shared marks grid `43c8f8df` (ONE `lib/shared/marks_grid/` kit both chains render; certified admin path untouched). EOS UX PASS; analyze 0; full suite **3711 / 2-known-UX-7 (0 new)**; +~40 tests. Residual (carried): raw `.when()` bulk migration.
>
> **P2-UX-3 (Tier-3 DS enforcement) ✅ 2026-07-08** (`64613e07`) — pure-additive gates (0 `lib/`): DS lint guard (blocks new raw-color/TextStyle via ratchet baseline) · contrast checker gate (WCAG AA light+dark) · 4 persona-shell goldens. analyze 0; +22 gates. CI-wiring leg owner/CI-gated.
>
> **P2-UX-4 (accessibility pass) ✅ 2026-07-08** (`d412b78b`) — a11y verification+lint layer + genuine gap fixes; **closes UX-7** (2 known failures resolved). Semantics (app-bar button labelled) · tap-target 48dp lint + guideline · dynamic-type 1.3× dense-grid clamp · rendered contrast audit beyond the token-pair gate (fixed a dark-mode badge ≈1.7:1 failure). analyze 0; suite 3756/0-fail; goldens 70/70 unchanged; +21 tests. 1 tracked → P2-UX-5 (dark `tertiary` chip).
>
