...
> **✅ P2 CLOSED (2026-07-09).** Verify-first outcome: **built** the student-scope RLS read policy (`20260866`, the highest-value item), the report-card real school-name (parent+student), and the education-only vertical-pack picker gate (UI + backend); **removed** the one genuinely-orphaned `DynamicDashboardScreen` + 3 exclusive providers; **fixed** a DS-enforcement regression the P0/P1 wave had introduced. The remaining "dead code" candidates (vault-rotate, school-calendar, social router, memories/analytics, setup-wizard GET, widgets/data/refresh, the reachable catalog widget) were verified **reachable + RBAC-gated + no-UI-yet** (several certified/contract-tested) → **KEPT**, not churned. Full detail + evidence: `docs/GAP_SWEEP_CERTIFICATION.md`.
