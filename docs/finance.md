...
| Tier | Screen | Rule |
|------|--------|------|
| **Operational** | FN-01 | Full transaction control — source of truth for finance KPIs |
| **Executive** | MG-05 | Read-only embed; drills here |
| **Portfolio** | DR-04 | Multi-school aggregate; no PII |

FN-11 reports: canonical definitions in **Reports.md** RPT-FN-*. FN-10: primary viewer for **Audit.md** platform events.
...
## 19. AI Finance Copilot

**Component:** `AI/FinanceCopilot` — dock on FN-01, FN-02, FN-03, FN-05, FN-09 (AR-059)

### Scope

School finance data only — scoped to `school_id`. No cross-school queries (use Director DR-04 for chain).

### Insight Cards (FN-01)

| Card | Data source | Action |
|------|-------------|--------|
| Defaulter risk forecast | FN-03 + historical | Open FN-03 filtered |
| Collection forecast | FN-02 trends | Chart embed |
| Expense anomaly | FN-05 patterns | Flag rows |
| Payroll advisory | FN-06 draft | Link MG-03 if pending |
| Budget burn | FN-09 | Drill FN-05 |

### Example Prompts

- "Which classes have the highest fee defaults?"
- "Forecast collection for this month"
- "Flag unusual expenses this week"
- "Summarize pending approvals affecting cash flow"

### Actions

Send bulk reminder · Export defaulter list · Suggest payment plan · Ask Management to approve

### Restrictions

Cannot modify fee structures without explicit navigation to FN-02 config · no payroll execute via AI — user confirm only

---

## 20. Cross-Module Links
