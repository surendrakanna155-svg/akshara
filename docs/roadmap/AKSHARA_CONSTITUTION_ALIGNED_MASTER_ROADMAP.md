...
## 5.6 PROGRAM ASIP — AI SUPPORT INTELLIGENCE PLATFORM 🟡 (Constitution Parts 6 · 7B · 8) — *parallel lane, never gates the ERP*
...
| ID | Workstream | State (evidence) | Pri |
|---|---|---|---|
| **ASIP-1** | **School Incident Reporting** — report an issue with only Description + Screenshot (+ optional recording); `public_ref`; my-incidents; conversation | 🔶 **BUILT + GREEN (Phase 1)** — backend + Flutter client; 32 deno + 6 flutter tests, analyze 0; live-cert owner-gated. Reuses storage presign, communication, audit, RBAC | **P1** |
| **ASIP-2** | **Automatic Evidence Capture** — client auto-collects app version/device/OS/session/route/module/correlation-ids/breadcrumbs/recent-API-calls; server enriches from `listAuditEvents` + workflow/approval state → PII-minimized evidence snapshot | 🔶 **BUILT + GREEN (Phase 1)** — net-new client capture backbone + first real binary upload pipeline; PII-minimized snapshot; reuses `ErrorReportingService`/`CorrelationIdInterceptor`/`AuditLogger` | **P1** |
| **ASIP-3** | **AI Incident Package** — deterministic assembly of all evidence + governed LLM enrichment (categorization, severity suggestion, summary, likely root cause, suggested next steps); **human-approve** before any use | 🔶 **BUILT + GREEN (Phase 1)** — deterministic-first; routes through `callModelGateway` (spend cap/rate/cache/timeout/telemetry/fallback); complete even when the model declines | **P1** |
| **ASIP-4** | **Incident Clustering / Dedup / Similar-incident** — "many schools, one incident": shared-incident detection, link affected tickets, investigate once/resolve many | ⚪ **owner-gated (Phase 2, Decision A)** — cross-tenant; uses `ai_semantic_cache_embeddings`/pgvector for similarity | P1 |
| **ASIP-5** | **AI Support Investigation** — root-cause analysis, screenshot understanding, log summarization, suggested fixes, config recommendations, KB search, previous-incident matching | ⚪ **owner-gated (Phase 2)** — governed gateway; deterministic-first; explainable | P1 |
| **ASIP-6** | **Support Workspace** — one place: ticket, conversation, screenshots, timeline, logs, audit events, API traces, AI diagnosis, suggested resolution, internal notes, school-visible replies, assignment, escalation, resolution evidence | ⚪ **owner-gated (Phase 2, Decision B — web frozen)** — surface decision required | P1 |
| **ASIP-7** | **Engineering Handoff** — generate a complete Engineering Incident Package (all investigation evidence) so engineers begin fixing immediately | ⚪ **build (Phase 2)** — deterministic export of evidence + AI findings | P2 |
...
- **Dependencies:** governed AI gateway (W7) for all model use; storage/communication/approval primitives; **W0 convergence** for final integration/deploy. **Owner-gated:** Decision A (cross-tenant access model — recommend snapshot/mirror), Decision B (support workspace surface — recommend scoped web unfreeze), live-cert + deploy authorization.
