...
> - **ASIP-1…7:** 🟩 **PRODUCTION CERTIFIED + LIVE** on the pilot (both phases deployed additively + **live cert 18/18 test + 18/18 production smoke**) — cert `docs/SUPPORT_INTELLIGENCE_PLATFORM_CERTIFICATION.md`. **Not voided by the final audit.**
...
> - **🟡 FINAL INDEPENDENT CERTIFICATION AUDIT (2026-07-21): CONDITIONAL — CERTIFIED WITH CONDITIONS (owner-accepted).** 14-dimension multi-agent audit + adversarial verification. **0 P0** (no Part 7B auto-fail; security-RLS + SECURITY-DEFINER both 9/10; org wall holds; ASIP-8 adds no new definer). Overall ≈7.4/10. Authoritative record: `docs/support-intelligence/ASIP_FINAL_CERTIFICATION_AUDIT_2026-07-21.md`.
> - **▶ RESUME POINT — the 5 mandatory pre-deployment conditions are the ONLY open AI Support engineering items. Execute from `docs/support-intelligence/ASIP_REMEDIATION_REGISTER.md` (ASIP-RC-1…RC-5) — do NOT re-run the audit.** Remediation = **PLANNING ONLY (nothing implemented).**

| Condition | Audit ID | Mandatory pre-deploy fix | Blocks |
|---|---|---|---|
| **ASIP-RC-1** | P1-D | Migration `20260920000060_support_kb.sql` version **collides** with trunk `…000060_finance_recovery_minor_backfill` (trunk band now uses `…060/062/070/080/090` via PROGRAM ICA) → **renumber to the next free slot ≥ `20260920000100`** (verify ceiling at fold-in) + update the hardcoded path in `support_kb_migration_validation_test.ts`. **DO FIRST.** | Trunk fold-in + deploy (hard blocker) |
| **ASIP-RC-2** | P1-A | KB title not clamped to the DB `CHECK(≤400)` → a long title rolls back the school-facing resolution; clamp + isolate learn from the resolve write. | ASIP-8 correctness |
| **ASIP-RC-3** | P1-B | `/support-status=resolved` skips propagate/notify/learn (divergent resolve path); remove `resolved` from that transition or route it through `/resolve`. | Workflow integrity |
| **ASIP-RC-4** | P1-C | `handlePlatformResolve` not idempotent → duplicate reporter notification + inflated KB count; skip already-resolved targets. | Workflow integrity |
| **ASIP-RC-5** | P1-E | Best-effort mirror has no reconciliation (comment describes a non-existent path); add a reconcile path + fix the comment. | Ops robustness |

> - Plus **16 P2 + 19 P3** backlog (audit report §5) — do NOT gate certification.
> - **Production certification of ASIP-8:** ⏸ pending **the 5 conditions above (RC-1 first) AND** the approved deployment gate — then renumber-and-apply the KB migration + run the live 23/23. **Intentionally deferred to the unified ERP pilot deployment phase** (do NOT deploy/activate ASIP-8 independently). Other Phase-2 deploy-gated tail (principal-phone seeding) likewise pending that phase.
...
| **ASIP-8** | **Continuous Learning** — resolved incidents feed a knowledge base that improves future diagnosis/recommendations | 🟢 **BUILT + GREEN (Phase 2, 2026-07-21) — live-run owner/deploy-gated.** Resolve distils a `support_kb_article` keyed by the cluster fingerprint (mirror-domain, same RLS, **no SECURITY DEFINER**); recall is deterministic-first (exact/related) with an OPTIONAL dormant-safe pgvector layer; an exact match leads the AI diagnosis with the proven fix. Console `/support-console/kb` + recall panel. **65 deno + 152 web tests, deno check + tsc + build clean; live-cert extended to 23 checks (hermetic).** Deploy migration `…060` + live 23/23 run = owner-gated (same as Phase 2). | P2 |
...
- **2026-07-21 — ASIP CONDITIONAL CERTIFICATION → OWNER-ACCEPTED; remediation register created.** Owner accepted the CONDITIONAL (Certified-with-Conditions) verdict. The 5 confirmed P1s are recorded as **mandatory pre-deployment conditions ASIP-RC-1…RC-5** in the new SSOT `docs/support-intelligence/ASIP_REMEDIATION_REGISTER.md` (full traceability + planning-only fixes + a RESUME POINT so future ASIP work starts from the conditions without re-running the audit). The audit report `ASIP_FINAL_CERTIFICATION_AUDIT_2026-07-21.md` is preserved as the authoritative record. **Correction:** RC-1's renumber target is now **≥ `20260920000100`** — the trunk band advanced under PROGRAM ICA (slots `…060/062/070/080/090` consumed), so the earlier `…090` suggestion is superseded. **No code/deploy/production change; remediation remains PLANNING ONLY.**
- **2026-07-21 — ASIP FINAL CERTIFICATION AUDIT → 🟡 CONDITIONAL (CERTIFIED WITH CONDITIONS).** Independent 14-dimension multi-agent audit with adversarial per-finding verification (62 agents). **0 P0** (no Part 7B auto-fail; tenant isolation + SECURITY DEFINER both 9/10; ASIP-8 adds no new definer); overall ≈7.4/10. **5 confirmed P1 conditions** gate ASIP-8 production certification + canonical-trunk fold-in — hardest is **P1-D: migration `20260920000060` version collision with the trunk (renumber to `…000090` before fold-in)**; also P1-A KB-title-unclamped-rolls-back-resolve, P1-B divergent `/support-status=resolved`, P1-C non-idempotent resolve, P1-E mirror-no-reconciliation. 16 P2 + 19 P3 backlog. ASIP-1…7 PRODUCTION CERTIFIED + LIVE **not voided**. Report: `docs/support-intelligence/ASIP_FINAL_CERTIFICATION_AUDIT_2026-07-21.md`. **Remediation is PLANNING ONLY — nothing implemented; no deploy/production change.** Conditions fold into the unified ERP pilot deployment phase.
- **2026-07-21 — ASIP-8 (Continuous Learning / KB) → 🟢 BUILT + GREEN; live-run owner-gated.** The last open ASIP roadmap item built on the certified head. Purely additive: migration `20260920000060` (`support_kb_article` PLATFORM_ORG-walled with the mirror's RLS + a defensive, dormant-safe `support_kb_embedding`), two net-new `_shared/support` modules reusing the existing clustering fingerprint + governed embeddings client (no new engines, **no new `SECURITY DEFINER`** — the org wall is untouched), KB wiring into resolve/investigate/detail + `GET /support/platform/kb`, and a `/support-console/kb` web surface (B1 scoped unfreeze). No existing row deleted/re-scoped/weakened. **Verification: 65 deno + 152 web tests, `deno check`/`tsc`/`vite build` clean; live-cert extended to 23 hermetic checks.** Deploy of `…060` + the live 23/23 run remain owner/deploy-gated exactly like the rest of Phase 2. Design authority + build log: `docs/support-intelligence/ASIP_DESIGN.md`.
