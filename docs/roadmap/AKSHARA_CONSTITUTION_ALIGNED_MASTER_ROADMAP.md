...
| **ASIP-6** | **Support Workspace** — one place: ticket, conversation, evidence, timeline, AI diagnosis, suggested resolution, internal notes, assignment, escalation, resolution | 🔶 **BUILT + GREEN (backend + web console)** — `/support/platform/*` API (45 deno tests) + React `/support-console` (B1 scoped unfreeze; build PASS, 147 web tests). Live activation owner/deploy-gated | P1 |
