...
- **Wave:** **P1-PROD-9 — C11 · Admissions / Front-office productivity (ADM-1, ADM-2, ADM-3, ADM-4, ADM-5)** — 🔵 next up. (**C3** defers on GA-1 live; **C6** defers — HWK-1 owner schema change; **C4/C5/C7/C8/C9/C10** ✅ complete.)
- **Sequencing:** C11 depends on **C0 (XCT-1)**. ⚠ **Owner decision surfaced (👤 in queue): Admissions approval maker-checker (SoD on admission approval)** — [[akshara-decision-queue]]; ADM-3 bulk lead actions must not create placeholder-junk rows (ADM-5). **P2-UX-1 ∥-eligible.** **P1-CODE-4 (Identity) stays owner-gated.** Live lane stays owner-deferred.

### P1-PROD-9 tasks (this wave) — findings ADM-1..5
> ⚠ **Discovery-first (every C-wave so far was mostly verify-not-rebuild):** an Admissions/CRM backend + screens exist (leads, follow-ups, applications). Assess coverage before writing; close only verified gaps. Exports ride the **XCT-1 shared pipeline**.

| Sub | Sev | What |
|---|---|---|
| **ADM-1** | P1 | Real admissions reports export. |
| **ADM-2** | P1 | Auto-log WhatsApp/call to the lead timeline. |
| **ADM-3** | P1 | Bulk lead actions (multi-select). |
| **ADM-4** | P1 | Inline actions on "Follow-ups due today". |
| **ADM-5** | P1 | "New Application" from a real lead picker (removes placeholder-junk rows). |

### EOS gate
- Scope: **FEATURE (Admissions)**. PASS required. **Completion (roadmap `FINAL_QA_ROADMAP.md:564`):** reports export real data; contact auto-logs to timeline; bulk lead actions persist; follow-up inline actions work; New Application uses a real lead (no placeholder junk). Regression: `flutter analyze` 0 · `flutter test` no NEW failures (2 known UX-7) · `deno test`+`deno check` green for any touched `supabase/functions/**`.
...
### Next
On P1-PROD-9 (C11) EOS PASS + commit → **C12 (Finance productivity — deps C1+C2, both done) / C13 (Academic-work productivity — deps C4+C6; C6 defers) / C14…** per the C-table (`FINAL_QA_ROADMAP.md` §Phase C), skipping deferred waves (**C3** GA-1 live, **C6** HWK-1 owner migration). Once the identity batch above is resolved → **P1-CODE-4**. Owner-scope tasks P1-CODE-6/7/8 remain 👤.
...
- [ ] Admissions reports export real data (ADM-1); contact auto-logs to the lead timeline (ADM-2); bulk lead actions persist (ADM-3); follow-ups-due inline actions work (ADM-4); New Application uses a real lead picker — no placeholder junk (ADM-5). Verify existing; close gaps.
- [ ] `/eos` FEATURE PASS; journal row + roadmap C11 ✅ + ledger ADM-1..5; advance this file to the next unblocked C-wave + refresh the dashboard.
