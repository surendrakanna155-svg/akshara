...
**Then (strict order):** CFC-1 Code Freeze Checklist (10 items, evidence per item) → FREEZE-1 Feature Freeze → P4 → P5 → P6-VAL-1 → P6-PILOT-1 → P6-BETA-1 (5–10 real schools) → P7 → P8.
   **CFC-1 pre-clearing (2026-07-13, `2147a51a`) — 5/10 pure-code items green:** **item 1** TODO/FIXME CLEAN · **item 3** fake/stub APIs CLEAN (opus honesty audit — all stubs labelled/disabled; WhatsApp returns honest `success:false`) · **item 4** debug-in-release FIXED (`!kReleaseMode`-guarded; `Debug*` gated to NoOp in prod) · **item 6** commented-out-code CLEAN · **item 7** temp-bypasses CLEAN. **👤 item 2 needs an owner/architect decision** (production `Hybrid*` repos soft-degrade to a mock/empty fallback on API failure → mock reachable in prod by design; is that acceptable at freeze or must prod hard-fail?) + a release-flavor build proof. **⏳ Blocked:** item 5 (W2 flag → LIVE-1 ①) · item 8 (migration head==deployed) · item 9 (clean tree ALL lanes — curriculum dirty) · item 10 (no open P0/P1 — K-2 hardening). The gate closes only when all 10 are green in ONE sweep on ONE commit.
