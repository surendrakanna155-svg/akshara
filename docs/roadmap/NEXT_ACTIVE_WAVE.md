...
- **Wave:** **P1-PROD-7 — C9 · Operational Modules: Inventory, Library & Communication (INV-1/2, LIB-1/2, COM-1/2)** — 🔵 next up. (**C3** defers on GA-1 live; **C6** defers — HWK-1 owner schema change; **C4/C5/C7/C8** ✅ complete.)
- **Sequencing:** C9 depends on **C0 (XCT-1)**. Note: **inventory stock governance is owner-decided** ([[inventory-stock-governance-decision]] — value-reducing movements = maker-checker; hard-block negative stock; immutable ledger); **COM-1/COM-2 were largely built in the communication wave** (delivery/read report, audience segments — verify). **P2-UX-1 ∥-eligible.** **P1-CODE-4 (Identity) stays owner-gated.** Live lane stays owner-deferred.

### P1-PROD-7 tasks (this wave) — findings INV-1/2, LIB-1/2, COM-1/2
> ⚠ **Discovery-first (every C-wave so far was mostly verify-not-rebuild):** inventory + library + communication backends/screens exist. Assess coverage before writing; close only verified gaps. Exports ride the **XCT-1 shared pipeline**. **Inventory money/governance boundary:** respect [[inventory-stock-governance-decision]] (value-reducing = maker-checker; negative-stock hard-block; immutable posted slips + stock_movements ledger).

| Sub | Sev | What |
|---|---|---|
| **INV-1** | P1 | Stock issue/consumption with issue slip (stock can go *down*). |
| **INV-2** | P1 | Consumable registry + reorder-level CRUD. |
| **LIB-1** | P1 | One-click overdue list + export. |
| **LIB-2** | P1 | Catalog edit/delete + CSV bulk import. |
| **COM-1** | P1 | Per-broadcast delivery & read report + CSV export (likely built — verify). |
| **COM-2** | P1 | Audience picker (class/section) + saved segments (likely built — verify). |

### EOS gate
- Scope: **FEATURE (Operations)**. PASS required. **Completion (roadmap `FINAL_QA_ROADMAP.md:559`):** issue-slip persists; catalog edit/import works; broadcast delivery/read report exports; class/section audience resolves. **Money/governance tripwire:** inventory value-reducing movements follow maker-checker + immutable ledger ([[inventory-stock-governance-decision]]); no negative stock. Regression: `flutter analyze` 0 · `flutter test` no NEW failures (2 known UX-7) · `deno test`+`deno check` green for any touched `supabase/functions/**`.
...
- [ ] INV-1 issue-slip persists (stock goes down, governance-respecting); INV-2 consumable registry + reorder-level CRUD works.
- [ ] LIB-1 overdue list exports; LIB-2 catalog edit/delete + CSV bulk import works.
- [ ] COM-1 per-broadcast delivery/read report exports; COM-2 class/section audience resolves + saved segments (verify existing).
- [ ] `/eos` FEATURE PASS; journal row + roadmap C9 ✅ + ledger INV-1/2·LIB-1/2·COM-1/2; advance this file to C10 + refresh the dashboard.
