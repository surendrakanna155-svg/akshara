...
| 4 | Weekly Timetable (TA-03) + Today's classes (TCH-1/TCH-4) | Premium canvas on all states of both (dropped a now-unused import on the timetable screen) | **No** — period lists / counts, no headline % | analyze clean; timetable suite + today-schedule + tch1/3/4 + module-screens **+18**; goldens 4 | `<pending>` |
