...
**Trend per round: 1 → 1 → 0 → (1 P0+1 P1) → 2 → 0. Each round sweeps a DISTINCT domain (money-race → AI → isolation → prod-correctness → RBAC-coverage); 3 of 6 rounds found real defects (all fixed+certified incl. 1 P0), the last two of those domains + the AI/RBAC domains are now CLEAN. The fake-DB test blind spot (GROUP BY, grants, JOINs) is the recurring theme: real-Postgres probing + prod logs find what 3484 green tests can't. Remaining domains for future rounds: performance (unbounded/N+1), served-vs-inventory route completeness, Flutter client, DR/recovery.**
