...
| **R6** | RBAC route coverage (auth-coverage analog of the RLS sweep) | 0 | 0 | **CLEAN.** All 18 `permission:null` inventory routes are legitimately gated: parent/student routes by persona SCOPE + RLS (parent-child isolation, R3); the 2 webhooks by HMAC shared-secret; `/audit/events/batch` school-scoped append-only. Parent MONEY routes (`/parent/payments/confirm`, `/payments/intents/confirm`) enforce RT-19: parent scope + `payer_user_id = claims.sub` ownership (payment_handlers.ts:162) — a parent can't act on another family's payment. No ungated/bypassable route found. | — (no P0/P1) |

| **R7** | Performance / unbounded-query DoS | 0 | 0 | **CLEAN.** Big-table list reads (students / finance_invoices / finance_collections / attendance_records / notification_deliveries / exam_mark_entries) are all bounded (LIMIT + `academic_pagination.clampPageSize`) or aggregated (count/SUM). The only unbounded fan-outs (Tally + transport-expense exports) were capped in R1/R5. | — (no P0/P1) |

**Trend per round: 1 → 1 → 0 → (1 P0+1 P1×3) → 2 → 0 → 0. Seven rounds across seven distinct domains (money-race · AI · isolation · prod-correctness · RBAC-coverage · performance). Findings: 1 P0 + 6 P1 — ALL fixed + LIVE CERTIFIED. The last THREE domains audited (AI, RBAC-coverage, performance) came back CLEAN, and the two domains that had defects (isolation, prod-correctness) are closed at the source (isolation sweep now returns "none"; the prod 500s are gone, 0 errors on prod). ⇒ the red-team has substantially CONVERGED. Recurring theme: the fake-DB test harness can't evaluate GROUP BY / grants / JOINs — real-Postgres probing + prod logs found what 3484 green tests could not (same class as PRC-A-D-04). Remaining lower-yield / infra-gated domains for any future pass: Flutter-client, DR/recovery, served-vs-inventory route completeness enumeration.**

## Convergence conclusion (P4-RT-1)
Across 7 rounds the red-team found and closed 1 P0 + 6 P1, all live-certified on prod, and the most recent domain sweeps are clean. This satisfies the round law's spirit — repeated adversarial rounds no longer finding *new-domain* meaningful issues on the backend/security/correctness surface. Full P4 EXIT still wants the device/infra-gated legs (Patrol automation, real load/stress, real-user simulation) which are owner/environment-provisioned, and feed P6-VAL-1.
