...
**Deferred deployment queue (execute during the unified ERP pilot deployment phase — do NOT run standalone):** apply migration `20260920000060_support_kb.sql` (additive; head → `…060`); redeploy the additive `_shared/support/*` KB modules + the web `/support-console/kb` surface; run `scripts/qa/live_cert_asip_vps.sh` for the live **23/23** (test + production smoke); then mark ASIP-8 PRODUCTION CERTIFIED. Preserve all additive ASIP files on the shared edge bind-mount across any W0 redeploy.
