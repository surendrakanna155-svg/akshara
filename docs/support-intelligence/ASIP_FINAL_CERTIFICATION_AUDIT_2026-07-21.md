...
- **Remediation (plan):** before fold-in, renumber the ASIP-8 migration to a version above the trunk's highest `20260920` slot — e.g. `20260920000090_support_kb.sql` (after `…000080`) — keeping versions unique and strictly monotonic; in the **same** change update the hardcoded path + comment in `support_kb_migration_validation_test.ts` (or switch it to a `migrations/*_support_kb.sql` glob). This is the single most important pre-deploy condition and belongs in the unified ERP pilot deployment phase.
