    ...
    for (final a in allocations) {
      ...
      records.insert(
        ...
        TransportAttendanceRecord(
          ...
          // BUS-006: this row is seeded from the allocation's PICKUP stop, so
          // the pickup time is the scheduled time — `scheduledTime` no longer
          // exists on TransportStop.
    ...
    final resequenced = [
      for (var i = 0; i < stops.length; i++)
        TransportStop(
          ...
          name: stops[i].name,
          sequence: i + 1,
          pickupTime: stops[i].pickupTime,
          dropTime: stops[i].dropTime,
  ...
  Future<TransportRoute> updateStop({
    ...
    return _mutateRouteStops(request.routeId, (stops) {
      ...
      stops[i] = TransportStop(
        ...
        sequence: s.sequence,
        pickupTime: request.pickupTime ?? s.pickupTime,
        dropTime: request.dropTime ?? s.dropTime,
        status: s.status,
        latitude: s.latitude,
