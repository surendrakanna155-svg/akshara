...
class HrSettingsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final isLoading = ref.watch(hrSettingsLoadingProvider);
    final isError = ref.watch(hrSettingsErrorProvider);
    final data = ref.watch(hrSettingsProvider);

    return HrModuleScaffold(
      screen: HrScreen.settings,
      showFilterBar: false,
      body: _buildBody(
        context,
        isLoading: isLoading,
  ...
  Widget _buildBody(
    ...
    required HrSettingsData? data,
  }) {
    if (isLoading) {
      return const Padding(
        padding: EdgeInsets.symmetric(vertical: AksharaSpacing.s12),
        child: AksharaLoadingState(semanticLabel: 'Loading HR settings'),
      );
    }

    if (isError) {
      return const AksharaErrorState(message: 'Unable to load HR settings.');
    }

    if (data == null) {
      return const AksharaErrorState(
        message: 'HR settings are not configured.',
      );
    }
    ...
    return Column(
      ...
      children: [
        const AksharaSectionHeader(title: 'HR settings'),
        Text(
          'Default department: ${data.defaultDepartment.name}',
...
class _SettingsSection extends StatelessWidget {
  ...
  final HrSettingsSection section;
