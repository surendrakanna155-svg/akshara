      ...
      body: lifecycle.when(
        ...
        data: (snapshot) => ListView(
          ...
          children: [
            ...
            procurement.when(
              ...
              data: (proc) => Column(
                ...
                children: [
                  ...
                  const Text('Procurement workflow', style: TextStyle(fontWeight: FontWeight.bold)),
            ...
            const Divider(height: 32),
            const Text('Recent lifecycle events', style: TextStyle(fontWeight: FontWeight.bold)),
            if (snapshot.recentEvents.isEmpty)
              const Padding(
                padding: EdgeInsets.symmetric(vertical: 24),
                child: AksharaEmptyState(message: 'No lifecycle events recorded yet'),
              )
            else
              ...snapshot.recentEvents.map(
                (e) => ListTile(
                  leading: Icon(_iconForEvent(e.eventType)),
                  title: Text('${e.eventType.label} — ${e.assetTag.isNotEmpty ? e.assetTag : e.assetId}'),
                  subtitle: Text(e.notes.isNotEmpty ? e.notes : e.recordedAt),
                ),
              ),
          ],
        ),
