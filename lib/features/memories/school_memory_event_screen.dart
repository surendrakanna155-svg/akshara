...
class SchoolMemoryEventScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SchoolMemoryEventScreen> createState() => _SchoolMemoryEventScreenState();
}

class _SchoolMemoryEventScreenState extends ConsumerState<SchoolMemoryEventScreen> {
  ...
  Future<void> _uploadMedia({required bool video}) async {
    ...
    try {
      ...
      final filename = video ? 'clip_${DateTime.now().millisecondsSinceEpoch}.mp4' : 'photo_${DateTime.now().millisecondsSinceEpoch}.jpg';
  ...
  Future<void> _shareMedia(SchoolMemoryMedia media) async {
    try {
      ...
      final link = await ref.read(schoolMemoriesRepositoryProvider).resolveShareLink(
            query: ref.read(phase5QueryProvider),
            shareToken: token,
          );
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: eventAsync.maybeWhen(data: (e) => Text(e.title), orElse: () => const Text('Event')),
        actions: [
          if (_uploading)
            const Padding(
                      ...
                      return InkWell(
                        onTap: () => Navigator.of(context).push(
                          MaterialPageRoute<void>(
                            builder: (_) => SchoolMemoryMediaViewer(media: media),
                          ),
                        ),
                        child: Stack(
                          fit: StackFit.expand,
                          children: [
                            DecoratedBox(
                              decoration: BoxDecoration(
                                ...
                                image: !media.isVideo && media.storageUrl.isNotEmpty
                                    ? DecorationImage(
