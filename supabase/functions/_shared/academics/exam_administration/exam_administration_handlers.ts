...
async function withAuth<T>(
  ...
  const denied = readOnly
    ? requireExamRead(auth.claims)
    : requireExamWrite(auth.claims);
...
export async function handleListExamMarks(
  ...
  return await withAuth(req, config, true, async (claims) => {
    const { organizationId, schoolId } = tenantIds(claims);
    const rows = await withTenantContext(config, claims, (db) =>
      listExamMarks(db, organizationId, schoolId, examId)
    );
...
export async function handleUpdateExamMark(
  ...
  return await withAuth(req, config, false, async (claims) => {
    ...
    const { organizationId, schoolId } = tenantIds(claims);
    const row = await withTenantContext(config, claims, (db) =>
      updateExamMark(db, organizationId, schoolId, markEntryId, marksObtained)
    );
...
export async function handleVerifyCoordinator(
  ...
  return await withAuth(req, config, false, async (claims) => {
    ...
    const verifiedBy = String(
      body?.verifiedBy ?? body?.verified_by ?? claims.userId,
    ).trim();
