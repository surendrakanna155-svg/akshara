...
//
// ## API-100 / API-101 / API-102 — this file is no longer the whole inventory
//
// It used to be a hand-maintained data file with no code-derived source and no
// test comparing it to the routers, so **adding a route did not require touching
// it**. 246 mutating routes were absent (`POST /finance/collections`,
// `/finance/refunds`, every `/approvals/:id/{approve,reject,cancel}`, every
// `/academics/exams/*` mark write), and the RBAC suite iterated this same list
// and called the pure function `requirePermission` — it never dispatched. A route
// added with no gate therefore passed the entire suite by not being listed.
//
// The inventory is now TWO halves:
//   * [CURATED_RBAC_ROUTE_RULES] — this list. Hand-written annotations that carry
//     intent a probe cannot recover (the HMAC webhook's `permission: null`, the
//     parent/student persona-scope classifications, the "why" comments).
//   * `DERIVED_RBAC_ROUTE_RULES` — generated by asking the dispatcher which
//     routes exist and which permission each one enforces.
//
// [RBAC_ROUTE_INVENTORY] is their union, and the guard
// (`rbac_generated_inventory_test.ts`) re-derives the table every run and fails
// when a mutating route is missing, when a rule names a route that no longer
// exists, or when the checked-in derived half is stale. Hand-editing this list
// to close a gap is the wrong move — run the generator (§7.9 of the remediation
...
export interface RbacRouteRule {
  ...
  module: string;
  /**
   * True when the rule came from `scripts/rbac/rbac_inventory_report.ts` rather
   * than a human. Lets the unit-level suites (which assert properties of
   * `requirePermission` itself, e.g. "no other permission grants this route")
   * keep operating on the curated set, where a single named permission is the
   * contract — derived rules may front an `requireAnyPermission` OR-gate, for
   * which "exactly one permission grants it" is not the intended property.
   */
  derived?: boolean;
}

/** The hand-written half: annotations, persona scopes, and the `null` gates. */
export const CURATED_RBAC_ROUTE_RULES: RbacRouteRule[] = [
  ...
  // API-102: `POST /admissions/approvals/:id/approve` was declared here and NO
  // router answers it — the live path is `POST /admissions/approval/:id/approve`
  // (singular), which the derived half now carries. Removed by the generator's
  // reconciliation report, not by hand-reading.
  ...
  // DB6: the retention-policy read. Added recently and never registered — the
  // exact omission the generated half now makes impossible for a mutating route,
  // and which the reverse (rule → route) guard catches for a read.
  { method: "GET", path: "/audit/retention", permission: "viewManagement", scope: "school", module: "audit" },
  ...
  // API-102: `/predictions/fee-defaults` is declared by nobody — the live route
  // is `/predictions/fee-default` (singular). Removed via the reconciliation
  // report; the real path is covered by the derived half.
  { method: "GET", path: "/predictions/fee-default", permission: "viewFinance", scope: "school", module: "predictions" },
  ...
  // API-102: `/director/summary` is declared by nobody — the director router's
  // read surface starts at `/director/dashboard`. Removed via the reconciliation
  // report.
  { method: "GET", path: "/director/dashboard", permission: "viewDirectorPortal", scope: "organization", module: "director" },
...
/**
 * The full inventory: the curated annotations plus every route the dispatcher
 * actually serves. A curated rule WINS on collision — it carries the intent
 * (`permission: null` for the HMAC webhook, the persona scopes) that a probe
 * cannot recover.
 *
 * Consumers that want the completeness surface ("does every mutating route have
 * a gate / an audit path?") read this. Consumers asserting properties of
 * `requirePermission` itself read [CURATED_RBAC_ROUTE_RULES].
 */
export const RBAC_ROUTE_INVENTORY: RbacRouteRule[] = (() => {
  const key = (r: RbacRouteRule) =>
    `${r.method} ${r.path.replace(/:[A-Za-z0-9_]+/g, ":p")}`;
  const seen = new Set(CURATED_RBAC_ROUTE_RULES.map(key));
  const merged = [...CURATED_RBAC_ROUTE_RULES];
